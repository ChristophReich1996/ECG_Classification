...
class PhysioNetDataset(ECGDataset):
    """
    This class implements the PhysioNet dataset.
    https://physionet.org/content/challenge-2017/1.0.0/
    """

    def __init__(self, path: str, augmentation_pipeline: Optional[nn.Module] = None, spectrogram_length: int = 80,
                 spectrogram_shape: Tuple[int, int] = (128, 128), ecg_sequence_length: int = 18000,
                 ecg_window_size: int = 256, ecg_step: int = 256 - 32, normalize: bool = True, fs: int = 300) -> None:
        """
        Constructor method
        ...
        # Get labels
        ecg_labels = []
        with open(os.path.join(path, "REFERENCE.csv"), "r") as file:
            for line in file.readlines():
                ...
                if "N" in label:
                    ...
                    raise RuntimeError("Invalid label value detected!")
        # Get leads
        ecg_leads = [loadmat(data_sample)["val"].reshape(-1)
                     for data_sample in sorted(os.listdir(path)) if ".mat" in data_sample]
        # Call super constructor
        super(PhysioNetDataset, self).__init__(ecg_leads=ecg_leads, ecg_labels=ecg_labels,
                                               augmentation_pipeline=augmentation_pipeline,
                                               spectrogram_length=spectrogram_length,
                                               spectrogram_shape=spectrogram_shape,
                                               ecg_sequence_length=ecg_sequence_length,
                                               ecg_window_size=ecg_window_size,
                                               ecg_step=ecg_step,
                                               normalize=normalize,
                                               fs=fs)
